...
# make model ensamle predictions by changing random_state
rmse = []
r2 = []
mape = []
rmse_geo_NA = []
...
for i in range(0,100):
    ...
    prediction_df_latlon[f'Pb_208_207_rs{i}'] = global_preds

# Save the predictions
timestr = time.strftime("%Y%m%d-%H%M%S")
os.makedirs(f'Model_output/{timestr}_Pb87')
prediction_df_latlon.to_csv(f'Model_output/{timestr}_Pb87/XGBoost-87pb_ensemble_predictions.csv', index=False)
...
performance_metrics.to_csv(f'Model_output/{timestr}_Pb87/XGBoost-87pb_ensemble_performance.csv', index=False)

#Save copy of the script
os.system(f'cp Pb-87_make-ensemble.py Model_output/{timestr}_Pb87/Pb-87_make-ensemble.py')
